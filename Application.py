...
Title.grid(row=0, column=0, rowspan=1, padx=303, columnspan=3)  # pack(pady=40)

noTitle = Frame(root, bg='white')
noTitle.grid(row=1, column=1, pady=50, rowspan=2, columnspan=1,
             sticky=N)  # pack(side= BOTTOM, ipady=100,pady=40,anchor=CENTER,fill= Y)

Title1 = Frame(root, bg='white')
Title1.grid(row=1, column=0, ipady=100, rowspan=2, columnspan=1,
            sticky=N + S + W)  # pack(side=RIGHT,anchor=W,fill= Y,ipady=500)

Title2 = Frame(root, bg='white')
Title2.grid(row=1, column=2, rowspan=2, columnspan=1, sticky=N + S + E)  # pack(side=LEFT,padx=40,anchor=E,fill= Y)
...
def FLoginForm():
    ...
    # SLog_Button.bind('<Return>', FLogin)
...
def FLogin():
    ...
    btn_search = Button(TopForm, text="STUDENT RESET PASSWORD", bg='#1E8FFF', fg='white', font=('calibri', 16, 'bold'),
                        width=30, command=Reset)  # , command=Search
...
Image_path = "C:\\Users\\ASUS\\Downloads\\4.PNG"                                                   #path to of the image
# Image_path="C:\\Users\\Aneesha Panigrahi\\Desktop\\front end\\4.PNG"
